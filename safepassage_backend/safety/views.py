...
def _nearest_resource_for_types(lat, lng, target_types, radius_steps=(8, 20, 50), limit_per_radius=40):
    normalized_types = set(target_types)
    for radius_km in radius_steps:
        resources = _build_nearby_resource_payload(lat, lng, radius_km=radius_km, limit=limit_per_radius)
        match = next((item for item in resources if item.get("type") in normalized_types), None)
        if match:
            return match
    return None


def _select_priority_safety_resources(resources, limit=4, nearest_hospital=None, nearest_police=None):
    prioritized = []
    seen = set()
    ...
    def add_resource(resource):
        if not resource:
            return
        key = (
            (resource.get("name") or "").strip().lower(),
            round(float(resource.get("latitude", 0)), 5),
            round(float(resource.get("longitude", 0)), 5),
        )
        if key in seen:
            return
        seen.add(key)
    ...
    nearest_hospital = nearest_hospital or next((item for item in resources if item.get("type") == "Hospital"), None)
    nearest_police = nearest_police or next((item for item in resources if item.get("type") == "Police Station"), None)

    add_resource(nearest_hospital)
    add_resource(nearest_police)
...
def _build_risk_payload(lat, lng):
    ...
    nearest_hospital = _nearest_resource_for_types(lat, lng, {"Hospital"})
    nearest_police_station = _nearest_resource_for_types(lat, lng, {"Police Station"})
    ...
    resources = _select_priority_safety_resources(
        nearby_resources,
        limit=4,
        nearest_hospital=nearest_hospital,
        nearest_police=nearest_police_station,
    )
...
def _build_route_destination_catalog():
    ...
    common_india_destinations = [
        ("Jaipur", 26.912434, 75.787271, "City Destination", "Rajasthan, India"),
        ("New Delhi", 28.613939, 77.209023, "City Destination", "Delhi, India"),
        ("Mumbai", 19.076090, 72.877426, "City Destination", "Maharashtra, India"),
        ("Bengaluru", 12.971599, 77.594566, "City Destination", "Karnataka, India"),
        ("Chennai", 13.082680, 80.270718, "City Destination", "Tamil Nadu, India"),
        ("Hyderabad", 17.385044, 78.486671, "City Destination", "Telangana, India"),
        ("Kolkata", 22.572646, 88.363895, "City Destination", "West Bengal, India"),
        ("Pune", 18.520430, 73.856744, "City Destination", "Maharashtra, India"),
        ("Ahmedabad", 23.022505, 72.571362, "City Destination", "Gujarat, India"),
        ("Surat", 21.170240, 72.831062, "City Destination", "Gujarat, India"),
        ("Lucknow", 26.846694, 80.946166, "City Destination", "Uttar Pradesh, India"),
        ("Varanasi", 25.317645, 82.973915, "City Destination", "Uttar Pradesh, India"),
        ("Udaipur", 24.585445, 73.712479, "City Destination", "Rajasthan, India"),
        ("Goa", 15.299326, 74.123996, "State Destination", "Goa, India"),
        ("Kochi", 9.931233, 76.267303, "City Destination", "Kerala, India"),
        ("Fort Kochi", 9.966667, 76.242462, "Heritage Destination", "Kochi, Kerala, India"),
        ("Ernakulam", 9.981636, 76.299884, "City Destination", "Kerala, India"),
        ("Thrippunithura", 9.949721, 76.350357, "Town Destination", "Kerala, India"),
        ("Kolenchery", 9.979389, 76.474918, "Town Destination", "Kerala, India"),
        ("Sree Narayana Gurukulam College of Engineering", 10.009454, 76.452675, "College Landmark", "Kadayiruppu, Kolenchery, Kerala, India"),
        ("Munnar", 10.088933, 77.059525, "Hill Destination", "Kerala, India"),
        ("Alappuzha", 9.498066, 76.338849, "Backwater Destination", "Kerala, India"),
        ("Varkala", 8.737869, 76.716335, "Beach Destination", "Kerala, India"),
        ("Kovalam", 8.398801, 76.978344, "Beach Destination", "Kerala, India"),
        ("Thekkady", 9.595193, 77.161938, "Wildlife Destination", "Kerala, India"),
        ("Wayanad", 11.685357, 76.131996, "Hill Destination", "Kerala, India"),
        ("Thrissur", 10.527641, 76.214435, "City Destination", "Kerala, India"),
        ("Kozhikode", 11.258753, 75.780411, "City Destination", "Kerala, India"),
        ("Thiruvananthapuram", 8.524139, 76.936638, "City Destination", "Kerala, India"),
        ("Guruvayur", 10.594302, 76.041100, "Pilgrimage Destination", "Kerala, India"),
        ("Mysuru", 12.295810, 76.639381, "City Destination", "Karnataka, India"),
        ("Ooty", 11.406414, 76.693244, "Hill Destination", "Tamil Nadu, India"),
        ("Madurai", 9.925201, 78.119774, "City Destination", "Tamil Nadu, India"),
        ("Manali", 32.243187, 77.189176, "Hill Destination", "Himachal Pradesh, India"),
        ("Shimla", 31.104815, 77.173403, "Hill Destination", "Himachal Pradesh, India"),
    ]
    for index, (name, latitude, longitude, kind, description) in enumerate(common_india_destinations, start=1):
        destinations.append(
            {
                "id": f"india-city-{index}",
                "name": name,
                "kind": kind,
...
def _featured_india_places(limit=8):
    featured = []
    seen = set()
    preferred_names = [
        "Kochi",
    ...
    catalog = _build_route_destination_catalog()
    lookup = {item["name"]: item for item in catalog}

    for name in preferred_names:
        destination = lookup.get(name)
        if not destination:
            continue
        key = (
        ...
        featured.append(
            {
                ...
                "description": destination.get("description") or "India location",
                "latitude": destination.get("latitude"),
                "longitude": destination.get("longitude"),
                "source": "featured-place",
            }
        )
...
def _best_reasonable_safe_haven_waypoint(source_lat, source_lng, dest_lat, dest_lng, corridor_resources):
    straight_distance_km = _haversine_km(source_lat, source_lng, dest_lat, dest_lng)
    if straight_distance_km <= 0 or not corridor_resources:
        return None

    best_waypoint = None
    best_extra_km = None
    for haven, _ in corridor_resources[:5]:
        source_to_haven = _haversine_km(source_lat, source_lng, haven.latitude, haven.longitude)
        haven_to_destination = _haversine_km(haven.latitude, haven.longitude, dest_lat, dest_lng)
        total_distance_km = source_to_haven + haven_to_destination
        extra_distance_km = total_distance_km - straight_distance_km

        if source_to_haven < 0.3 or haven_to_destination < 0.3:
            continue
        if extra_distance_km <= 0.1:
            continue
        if extra_distance_km > min(4.0, straight_distance_km * 0.35):
            continue
        if total_distance_km > straight_distance_km * 1.35:
            continue

        if best_extra_km is None or extra_distance_km < best_extra_km:
            best_extra_km = extra_distance_km
            best_waypoint = (haven.latitude, haven.longitude)
...
def _build_safe_route_payload(user, source_lat, source_lng, dest_lat, dest_lng, destination_label=None):
    ...
    low_risk_haven_waypoint = _best_reasonable_safe_haven_waypoint(
        source_lat,
        source_lng,
        dest_lat,
        dest_lng,
    ...
    should_use_direct_fallback = straight_distance_km > 30 and not corridor_resources and not corridor_hotspots
    ...
    if should_use_direct_fallback:
        medium_route_points = [
            (source_lat, source_lng),
            (dest_lat, dest_lng),
        ]
        low_route_points = [(source_lat, source_lng)]
        if low_risk_haven_waypoint:
            low_route_points.append(low_risk_haven_waypoint)
        low_route_points.append((dest_lat, dest_lng))
    elif not corridor_hotspots:
        medium_route_points = [
            (source_lat, source_lng),
            fallback_detour,
            (dest_lat, dest_lng),
        ...
        if low_risk_haven_waypoint:
            low_route_points.append(low_risk_haven_waypoint)
        low_route_points.append(_perpendicular_detour_point(source_lat, source_lng, dest_lat, dest_lng, scale=0.24))
        low_route_points.append((dest_lat, dest_lng))
    else:
        ...
        if low_risk_haven_waypoint:
            low_route_points.append(low_risk_haven_waypoint)
        elif nearest_haven and corridor_resources[0][1] <= 12:
            low_route_points.append((nearest_haven.latitude, nearest_haven.longitude))
        low_route_points.append(strong_detour)
        low_route_points.append((dest_lat, dest_lng))
...
def _dispatch_emergency_alert(user, lat, lng, mode, request=None):
    ...
    unique_email_targets = []
    seen_emails = set()
    for target in email_targets:
        email_address = (target.get("email") or "").strip().lower()
        if not email_address or email_address in seen_emails:
            continue
        seen_emails.add(email_address)
        unique_email_targets.append({**target, "email": email_address})

    if not unique_email_targets and (user.email or "").strip():
        unique_email_targets.append(
            {
    ...
    sender = _notification_sender_email()
    for target in unique_email_targets:
        personalized_message = message
        if request:
            confirm_url = request.build_absolute_uri(reverse('api_sos_acknowledge', kwargs={'token': alert.token}))
            query_string = urlencode(
                {
                    "recipient_name": target.get("name") or "",
                    "recipient_email": target.get("email") or "",
                }
            )
            if query_string:
                confirm_url = f"{confirm_url}?{query_string}"
            personalized_message += (
                f"\n\n"
                f"----------------------------------------\n"
                f"CONFIRM RECEIPT:\n"
                f"If you have received this SOS alert, click the link below to send an acknowledgement back:\n"
                f"{confirm_url}\n"
                f"----------------------------------------"
            )
    ...
    if alert.acknowledged_at:
        dispatch_status_label = f"Received by {alert.acknowledged_by or 'emergency contact'}"
    elif unique_email_targets:
        dispatch_status_label = f"Sent to {len(unique_email_targets)} email contact(s)"
    elif contact_deliveries:
        dispatch_status_label = "Dispatched to saved emergency contacts"
    else:
        dispatch_status_label = "Saved without any configured emergency contact channels"
    ...
    return {
        ...
        "delivery_channels": {
            ...
            "email_contacts": len(unique_email_targets),
            "total_channel_dispatches": sms_contacts + whatsapp_contacts + len(unique_email_targets),
        },
...
def api_emergency(request):
    ...
    if request.user.role == "worker":
        worker_sos_setup = _worker_sos_setup_status(request.user)
        if not worker_sos_setup["is_ready"]:
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Complete employer details and add 2 emergency contacts before sending a worker SOS.",
                    "missing_requirements": worker_sos_setup["missing_requirements"],
                },
                status=400,
            )
...
def add_contact(request):
    ...
    try:
        if request.user.role == "worker" and EmergencyContact.objects.filter(user=request.user).count() >= 2:
            return JsonResponse({'success': False, 'error': 'Workers can save up to 2 emergency contacts on the SOS page.'})
...
WORKER_SHIFT_REMINDER_INTERVAL_MINUTES = 2
WORKER_SHIFT_REMINDER_COUNT = 3
WORKER_SHIFT_AUTO_SOS_MINUTES = WORKER_SHIFT_REMINDER_INTERVAL_MINUTES * (WORKER_SHIFT_REMINDER_COUNT + 1)
...
def _serialize_worker_shift_preferences(worker_profile, reference_dt=None):
    ...
    return {
        ...
        "reminder_interval_minutes": WORKER_SHIFT_REMINDER_INTERVAL_MINUTES,
        "reminder_count": WORKER_SHIFT_REMINDER_COUNT,
...
def _worker_sos_setup_status(user):
    worker_profile = _get_or_create_worker_profile(user)
    employer_name = (worker_profile.emergency_contact_name or "").strip()
    employer_phone = (worker_profile.emergency_contact_phone or "").strip()
    employer_email = (worker_profile.emergency_contact_email or "").strip()
    emergency_contacts = list(EmergencyContact.objects.filter(user=user).order_by("-is_primary", "name"))
    missing_requirements = []

    if not employer_name or not employer_phone or not employer_email:
        missing_requirements.append("Add employer name, phone, and email.")
    if len(emergency_contacts) < 2:
        missing_requirements.append("Add 2 emergency contacts.")

    return {
        "employer_contact_complete": bool(employer_name and employer_phone and employer_email),
        "employer_contact": {
            "name": employer_name,
        ...
        "emergency_contacts_remaining": max(2 - len(emergency_contacts), 0),
        "can_add_emergency_contacts": len(emergency_contacts) < 2,
...
def _build_worker_template_context(user, **extra_context):
    current_lat, current_lng = _resolve_user_coordinates(user)
    worker_profile = _get_or_create_worker_profile(user)
    worker_sos_setup = _worker_sos_setup_status(user)
    base_context = {
        ...
        "emergency_contacts": worker_sos_setup["emergency_contacts"],
        "emergency_contacts_count": worker_sos_setup["emergency_contacts_count"],
        "worker_sos_setup": worker_sos_setup,
...
def api_worker_sos_target(request):
    ...
    if not re.match(r"^\d{10}$", contact_phone):
        return JsonResponse(
            {"status": "error", "message": "Employer phone must be a valid 10-digit mobile number."},
            status=400,
        )
    ...
    worker_profile.save(update_fields=["emergency_contact_name", "emergency_contact_phone", "emergency_contact_email"])
    worker_sos_setup = _worker_sos_setup_status(request.user)

    return JsonResponse(
        {
            "status": "success",
            "message": "Employer SOS contact updated successfully.",
            "contact": {
                "name": worker_profile.emergency_contact_name,
            ...
            "setup": {
                "is_ready": worker_sos_setup["is_ready"],
                "emergency_contacts_count": worker_sos_setup["emergency_contacts_count"],
                "emergency_contacts_required": worker_sos_setup["emergency_contacts_required"],
                "missing_requirements": worker_sos_setup["missing_requirements"],
            },
        }
    )
...
def api_worker_shift_escalation(request):
    ...
    def _build_timed_escalation(reference_time, escalation_type, action_label, shift_id=None):
        minutes_overdue = max(int((now - reference_time).total_seconds() // 60), 0)
        schedule_payload = {
            "type": escalation_type,
            "minutes_overdue": minutes_overdue,
            "shift_id": shift_id,
            "scheduled_time": reference_time.isoformat(),
        ...
        reminder_cutoffs = [reminder_interval_minutes * index for index in range(1, reminder_count + 1)]
        for reminder_index, threshold in enumerate(reminder_cutoffs, start=1):
            next_threshold = (
                reminder_cutoffs[reminder_index]
                if reminder_index < reminder_count
                else auto_sos_after_minutes
            )
            if threshold <= minutes_overdue < next_threshold:
                return {
                    "escalation_level": f"reminder_{reminder_index}",
                    "message": (
                        f"It is time to {action_label}. Reminder {reminder_index} of {reminder_count}: "
                        f"your scheduled {action_label} is overdue by {minutes_overdue} minutes."
                    ),
                    "client_dispatch_required": False,
                    "auto_sos_dispatched": False,
                    "auto_sos_message": (
                        f"If there is still no response, SafePassage will send SOS after reminder {reminder_count}. "
                        f"Alerts are spaced {reminder_interval_minutes} minutes apart."
                    ),
                    **schedule_payload,
                }

        if minutes_overdue >= auto_sos_after_minutes:
            response_payload = {
                "escalation_level": "sos",
                "message": (
                    f"{action_label.title()} is overdue by {minutes_overdue} minutes. "
                    f"SafePassage sent three reminders at {reminder_interval_minutes}-minute intervals and is now escalating to SOS."
                ),
                **schedule_payload,
            }
            response_payload.update(_dispatch_shift_escalation_sos(request.user, reference_time, shift_type=action_label))
    ...
    active_shift_checkin = (
        CheckIn.objects.filter(shift=active_shift).order_by("-timestamp").first()
        if active_shift
        else None
    )
    scheduled_shift = _find_worker_shift_for_schedule(request.user, scheduled_start, scheduled_end)

    if active_shift and active_shift.end_time and now >= active_shift.end_time + timedelta(minutes=reminder_interval_minutes):
        response_payload = _build_timed_escalation(
            active_shift.end_time,
            escalation_type="end",
    ...
    if active_shift:
        checkin_reference_time = None
        if active_shift_checkin:
            checkin_reference_time = active_shift_checkin.timestamp
        elif active_shift.actual_start or active_shift.start_time:
            checkin_reference_time = active_shift.actual_start or active_shift.start_time

        if checkin_reference_time:
            next_checkin_due_at = checkin_reference_time + timedelta(minutes=30)
            if now >= next_checkin_due_at + timedelta(minutes=reminder_interval_minutes):
                response_payload = _build_timed_escalation(
                    next_checkin_due_at,
                    escalation_type="checkin",
    ...
    if not scheduled_shift and now >= scheduled_start + timedelta(minutes=reminder_interval_minutes):
        response_payload = _build_timed_escalation(
            scheduled_start,
            escalation_type="start",
            action_label="check-in",
            shift_id=None,
        )
        if response_payload:
        ...
        "alerts": [
            {
                ...
                "dispatch_status_label": (
                    f"Received by {a.acknowledged_by}"
                    if a.acknowledged_at and a.acknowledged_by
                    else "Received by emergency contact"
                    if a.acknowledged_at
                    else f"Sent to {sms_contacts + whatsapp_contacts} configured SMS/WhatsApp channel(s)"
                    if (sms_contacts + whatsapp_contacts) > 0
                    else "Alert sent"
                ),
...
def tourist_cultural_guide(request):
    starter_dos = _collect_cultural_entries("en", "do", limit=2)
    starter_donts = _collect_cultural_entries("en", "dont", limit=2)
    if not starter_dos:
        starter_dos = [
            _guidance_entry(
                "Read the local pace first",
                "Watch how people queue, speak with staff, and move through shared spaces before you join in.",
                source="starter-guide",
                source_label="Starter guide",
            )
        ]
    if not starter_donts:
        starter_donts = [
            _guidance_entry(
                "Avoid rushed decisions",
                "Do not hand over money, documents, or devices to unofficial helpers until you verify who they are.",
                source="starter-guide",
                source_label="Starter guide",
            )
        ]
    return render(
        ...
        'tourist/cultural_guide.html',
        {
            ...
            'featured_places': _featured_india_places(limit=8),
            'starter_dos': starter_dos,
            'starter_donts': starter_donts,
        },
    )
...
def api_sos_acknowledge(request, token):
    ...
    recipient_name = (request.GET.get("recipient_name") or "").strip()
    recipient_email = (request.GET.get("recipient_email") or "").strip()
    acknowledgement_sender = recipient_name or recipient_email or "Emergency contact"
    already_acknowledged = bool(alert.acknowledged_at)

    if not alert.acknowledged_at:
        alert.acknowledged_at = timezone.now()
        alert.acknowledged_by = acknowledgement_sender
        alert.status = "Acknowledged"
        alert.save()
    elif not alert.acknowledged_by and acknowledgement_sender:
        alert.acknowledged_by = acknowledgement_sender
        alert.save(update_fields=["acknowledged_by"])
    ...
    return render(
        ...
        {
            "alert": alert,
            "acknowledgement_sender": alert.acknowledged_by or acknowledgement_sender,
